...
class _SplashIntroPageState extends State<SplashIntroPage> {
  ...
  final List<Map<String, dynamic>> introFeatures = [
  {"icon": Icons.location_on, "title": "SUNWAY EXCLUSIVE"},
  {"icon": Icons.rate_review, "title": "REAL REVIEWS"},
  {"icon": Icons.mood, "title": "KNOW THE VIBE"},
  {"icon": Icons.smart_toy, "title": "ASK AND DISCOVER"},
];
  ...
  for (int i = 1; i <= introFeatures.length; i++) {
    await Future.delayed(const Duration(milliseconds: 750));

    if (mounted) {
      setState(() => visibleCount = i);
    }
  }

  // Pause AFTER last feature
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                ...List.generate(introFeatures.length, (index) {
                    final isVisible = index < visibleCount;
                    final feature = introFeatures[index];
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (_) => const NavigationBarExample(),
                          ),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: const Color(0xFFFF692E),
...
// ================== Main Navigation: Library + Chatbot Only ==================
...
class _NavigationBarExampleState extends State<NavigationBarExample> {
  int _selectedIndex = 0; // default = Library
  ...
  Widget build(BuildContext context) {
    final List<Widget> pages = <Widget>[
      ...
      ChatPage(onNavigateToLibrary: () => _onItemTapped(0)),
    ];
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        selectedItemColor: const Color(0xFFFF692E),
...
Widget _sentimentScaleCard(double avgSentiment) {
  final normalized = ((avgSentiment + 1) / 2).clamp(0.0, 1.0);
  final position = math.pow(normalized, 1.8).toDouble();
      ...
      children: [
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            final markerLeft = width * position - 8;
...
class _SearchLibraryPageState extends State<SearchLibraryPage> {
  ...
  setState(() {
    ...
    if (selectedSort == 'Top Delight') {
      filteredRestaurants.sort((a, b) => b.avgSentiment.compareTo(a.avgSentiment));
    } else if (selectedSort == 'Top Rated') {
      filteredRestaurants.sort((a, b) => b.avgStar.compareTo(a.avgStar));
    } else {
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 6),
                                  _sentimentScaleCard(restaurant.avgSentiment),
                                ],
                              ),
                            ),
                          ],
  ...
  return GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: selected ? const Color(0xFFFF692E) : const Color(0xFFFCF4B5),
        borderRadius: BorderRadius.circular(20),
      ),
      child: Text(
        label,
              ...
              child: Container(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (_currentStep == 4)
                      _buildOptionsGrid(["Muslim-friendly", "None"],
...
class _ResultsViewState extends State<ResultsView> {
  ...
  Future<void> fetchRecommendations() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://fyp-sunbites.onrender.com/recommend'),
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Column(
        children: [
          _buildHeader("SunBites Recommendations"),
          const SizedBox(height: 15),
          Expanded(
            child: AnimatedScale(
